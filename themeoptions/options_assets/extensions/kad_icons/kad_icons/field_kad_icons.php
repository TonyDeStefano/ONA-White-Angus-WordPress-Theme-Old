    ...
    class ReduxFramework_kad_icons {
        ...
        public function render() {
             $defaults = array(
                ...
                'content_title' => __ ( 'Icon', 'ona-white-angus' )
            );

             $this->field = wp_parse_args ( $this->field, $defaults );

           echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'ona-white-angus' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                foreach ($slides as $slide) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . esc_attr($x) . '">' . __('Upload Icon', 'ona-white-angus') . '</span>';

                    $hide = '';
                    if (empty($slide['url']) || $slide['url'] == '')
                        $hide = ' hide';

                    echo '<span class="button remove-image' . esc_attr($hide) . '" id="reset_' . esc_attr($x) . '" rel="' . esc_attr($slide['attachment_id']) . '">' . __('Remove', 'ona-white-angus') . '</span>';
                    echo '</div>' . "\n";
                   $icon_option = pinnacle_icon_list();
                        $placeholder = (isset($this->field['placeholder']['icon_o'])) ? esc_attr($this->field['placeholder']['icon_o']) : __( 'Select an Icon', 'ona-white-angus' );
                    ...
                    echo '<li><input type="hidden" id="' . esc_attr($this->field['id']) . '-url_' . esc_attr($x) . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][url]" value="' . esc_attr($slide['url']) . '" class="full-text upload" placeholder="'.__('URL', 'ona-white-angus').'" /></li>';
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-title_' . $x . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][title]" value="' . esc_attr($slide['title']) . '" placeholder="'.__('Title', 'ona-white-angus').'" class="full-text slide-title" /></li>';
                    echo '<li><textarea name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][description]" id="' . esc_attr($this->field['id']) . '-description_' . esc_attr($x) . '" placeholder="'.__('Description', 'ona-white-angus').'" class="large-text" rows="6">' . esc_attr($slide['description']) . '</textarea></li>';
                    echo '<li><input type="text" id="' . esc_attr($this->field['id']) . '-link_' . esc_attr($x) . '" name="' . esc_attr($this->field['name']) . '[' . esc_attr($x) . '][link]" value="' . esc_attr($slide['link']) . '" placeholder="'.__('Icon Link', 'ona-white-angus').'" class="full-text" /></li>';
                    ...
                    echo ' '.__('Open Link in New Tab/Window', 'ona-white-angus'). '</label></li>';
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __('Delete Icon', 'ona-white-angus') . '</a></li>';
            ...
            if ($x == 0) {
                ...
                echo '<span class="button remove-image' . esc_attr($hide) . '" id="reset_' . esc_attr($x) . '" rel="' . esc_attr($this->parent->args['opt_name']) . '[' . esc_attr($this->field['id']) . '][attachment_id]">' . __('Remove', 'ona-white-angus') . '</span>';
                ...
                $icon_option = pinnacle_icon_list(); 
                        ...
                        echo '</select>';                           
                echo '<ul id="' . esc_attr($this->field['id']) . '-ul" class="redux-slides-list">';
                $placeholder = (isset($this->field['placeholder']['url'])) ? esc_attr($this->field['placeholder']['url']) : __( 'URL', 'ona-white-angus' );
                ...
                $placeholder = (isset($this->field['placeholder']['title'])) ? esc_attr($this->field['placeholder']['title']) : __( 'Title', 'ona-white-angus' );
                ...
                $placeholder = (isset($this->field['placeholder']['description'])) ? esc_attr($this->field['placeholder']['description']) : __( 'Description', 'ona-white-angus' );
                ...
                $placeholder = (isset($this->field['placeholder']['link'])) ? esc_attr($this->field['placeholder']['link']) : __( 'Icon Link', 'ona-white-angus' );
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add2 kad_redux-icon-add button-primary" rel-id="' . esc_attr($this->field['id']) . '-ul" rel-name="' . esc_attr($this->field['name']) . '[title][]">' . __('Add Icon', 'ona-white-angus') . '</a><br/>';
